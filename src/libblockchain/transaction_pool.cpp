...
void transaction_pool::get_amounts(std::string const& key, std::vector<std::pair<std::string, coin>>& amounts, bool in_out) const
{
    for (auto &it : m_pimpl->m_transactions)
    {
        ...
        {
            amounts.push_back(std::pair<std::string, coin>(it.first, transfer.amount));
        }
        else if (!in_out && transfer.from == key)
        {
            coin temp = transfer.amount;
            temp += it.second.transaction_details.fee;
            amounts.push_back(std::pair<std::string, coin>(it.first, temp));
        }
    }
}
