...
// sponsoring stuff Ex
vector<string> action_owners(SponsorContentUnitEx const& sponsor_content_unit_ex)
{
    return { sponsor_content_unit_ex.sponsor_address };
}
vector<string> action_participants(SponsorContentUnitEx const& sponsor_content_unit_ex)
{
    return { sponsor_content_unit_ex.uri, sponsor_content_unit_ex.sponsor_address };
}
                     ...
                     bool/* check_complete*/)
{
    ...
    auto signed_authority = signed_transaction.authorizations.front().address;
    if (signed_authority != sponsor_content_unit_ex.sponsor_address)
        throw authority_exception(signed_authority, sponsor_content_unit_ex.sponsor_address);

    if (sponsor_content_unit_ex.hours == 0)
        throw wrong_data_exception("sponsor ex duration");
    if (sponsor_content_unit_ex.amount == Coin())
        throw wrong_data_exception("sponsor ex amount");
    if (sponsor_content_unit_ex.type == SponsorType::all || sponsor_content_unit_ex.type == SponsorType::global)
        throw wrong_data_exception("sponsor ex type");

    meshpp::public_key pb_key_from(sponsor_content_unit_ex.sponsor_address);

    string file_hash = meshpp::from_base58(sponsor_content_unit_ex.uri);
    if (file_hash.length() != 32)
        throw uri_exception(sponsor_content_unit_ex.uri, uri_exception::invalid);
}
...
{
    if (false == impl.m_documents.unit_exists(sponsor_content_unit_ex.uri))
        throw uri_exception(sponsor_content_unit_ex.uri, uri_exception::missing);
    ...
    */
    ...
    impl.m_state.decrease_balance(sponsor_content_unit_ex.sponsor_address,
                      ...
                      state_layer/* layer*/)
{
    ...
    for (auto const& temp_sponsored_reward : temp_sponsored_rewards)
        for (auto const& temp_sponsor_by_type : temp_sponsored_reward.second)
            ...
            {
                if (temp_detail.second.second == coin())
                    return false;
            }
                  ...
                  state_layer /*layer*/)
{
    ...
    for (auto const& temp_sponsored_reward : temp_sponsored_rewards)
        for (auto const& temp_sponsor_by_type : temp_sponsored_reward.second)
            for (auto const& temp_detail : temp_sponsor_by_type.second)
            {
                assert(temp_detail.second.second != coin());
                if (temp_detail.second.second == coin())
                    throw wrong_data_exception("invalid transaction hash: " + cancel_sponsor_content_unit.transaction_hash);
            }
}
    ...
    for (auto const& temp_sponsored_reward : temp_sponsored_rewards)
        for (auto const& temp_sponsor_by_type : temp_sponsored_reward.second)
            for (auto const& temp_detail : temp_sponsor_by_type.second)
            {
                assert(temp_detail.second.second != coin());
                if (temp_detail.second.second == coin())
                    throw std::logic_error("invalid transaction hash: " + cancel_sponsor_content_unit.transaction_hash);
            }
}
}
