...
public:
    ...
    std::unique_ptr<detail::sync_manager_context_detail> m_pimpl;
};
...
public:
    ...
    beltpp::packet process_storage_update_request(ManagerMessage::StorageUpdateRequest const& update, rpc& rpc_server);

    beltpp::packet send(ManagerMessage::Send const& send, rpc& rpc_server);

    beltpp::packet wait_response(std::string const& transaction_hash);

    sync_context start_new_import(rpc& rpc_server, std::string const& account);

    sync_context start_sync(rpc& rpc_server, std::unordered_set<std::string> const& set_accounts);
